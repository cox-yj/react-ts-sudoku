...
class Sudoku {
    ...
    constructor(difficulty) {
        ...
        let sudokuArr = this.createSudoku()
    ...
    createSudoku = (): SudokuItem[][] => {
        this.count = this.count + 1;

        let sudokuArr = this.initSudoku()
        const blocks = [1, 2, 3, 5, 6, 7]
        // 分块创建
        // for (let i = 0; i < 9; i++) {
        //     const blockArr = this.createSudokuBlock(i, sudokuArr);
        //     blockArr.forEach(item => {
        //         sudokuArr[item.row][item.columns] = item
        //     })
        // }
        blocks.forEach(block => {
            sudokuArr = this.createSudokuBlock(block, sudokuArr);
            // blockArr.forEach(item => {
            //     sudokuArr[item.row][item.columns] = item
            // })
